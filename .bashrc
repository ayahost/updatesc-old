...
if [[ $ssr_status == "running" ]] ; then
  status_ssr=" ${GREEN}Running${NC} "
else
  status_ssr="${RED}  Not Running ${NC} "
fi
...
if [[ $status_text == "active" ]] ; then
  ...
  status_sodosok="${RED}  Not Running ${NC} "
fi
...
if [[ $strgo == "active" ]]; then
  ...
  status_trgo="${RED}  Not Running ${NC} "
fi

# STATUS SERVICE L2TP
if [[ $l2tp_status == "running" ]]; then 
   status_l2tp=" ${GREEN}Running${NC} "
else
   status_l2tp="${RED}  Not Running${NC}  "
fi
...
if [[ $sstp_service == "running" ]]; then 
   ...
   status_sstp="${RED}  Not Running ${NC}"
fi
...
# STATUS OHP OpenVPN
if [[ $ohq == "running" ]]; then 
   sohq=" ${GREEN}Running ${NC}"
else
   sohq="${RED}  Not Running ${NC} "
fi
...
if [[ $wsopen == "running" ]]; then 
   swsopen=" ${GREEN}Running ${NC}" 
else
   swsopen="${RED}  Not Running ${NC}"
fi
...
#cowsay -f eyes "KERJA KITA PRESTASI BANGSA" | lolcat
#toilet -f smslant "AyaHost" -F gay

echo -e  " \033[1;33m Ketik \033[1;31m Menu ${mg}untuk memulai ${NC}"